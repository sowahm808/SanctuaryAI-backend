...
export class WorkflowsService {
  ...
  async list(user: FirebaseIdentity, area: string) { const cfg = this.cfg(area); const { organizationId } = await this.active(user, cfg.read); return { items: [], nextCursor: null, organizationId }; }

  async create(user: FirebaseIdentity, area: string, body: R) {
  ...
  async patch(user: FirebaseIdentity, area: string, id: string, body: R) {
    const cfg = this.cfg(area); const item = await this.item(user, cfg, id, cfg.update); this.assertRevision(item, body); this.assertUnlocked(item);
  ...
  async generate(user: FirebaseIdentity, area: string, id: string, body: R = {}) { const cfg = this.cfg(area); const item = await this.item(user, cfg, id, cfg.update); const sourceRevision = this.s(body.sourceRevision) || this.s(body.revision) || this.s(item.revision); return this.job(user, this.s(item.organizationId), `${area}_generate`, { id, ...body, sourceRevision }, sourceRevision); }
  ...
  private hasPermission(membership: R | undefined, permission: string) { const values = this.arr(membership?.permissions).map((value) => this.s(value)); const role = this.s(membership?.role); const [area, action] = permission.split("."); const legacyWrite = area && ["create", "update", "upload"].includes(action ?? "") ? `${area}.write` : ""; const legacyRead = action === "read" && values.includes("organizations.read"); const legacyApprove = action === "approve" && values.includes("approvals.review"); return values.includes(permission) || (legacyWrite ? values.includes(legacyWrite) : false) || legacyRead || legacyApprove || values.includes("admin") || ["ChurchAdministrator", "SuperAdministrator"].includes(role); }
  private assertRevision(item: R, body: R) { const revision = this.s(body.revision); if (revision && revision !== this.s(item.revision)) throw new ConflictException({ code: "revision_conflict", message: "The resource is stale.", currentRevision: this.s(item.revision) }); }
